...
redis_pool = aioredis.ConnectionPool(host='redis', port=6379, db=0, decode_responses=True)
...
async def send_notification(user_id: int, like_data: Dict) -> None:
    ...
    try:
        ...
        logger.info('Notification sent to user %s about like from %s', user_id, like_data['from_user_id'])
    except Exception as e:
