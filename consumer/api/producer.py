...
async def send_profile_request(user_id: int, request_data: Dict[str, Any]) -> None:
    ...
    try:
        ...
        async with connection:
            ...
            # Declare the queue
            queue = await channel.declare_queue(PROFILE_REQUEST_QUEUE, durable=True)

            # Prepare the message
            message_data = {'user_id': user_id, **request_data}

            # Create the message
            message = aio_pika.Message(
                body=json.dumps(message_data).encode(), delivery_mode=aio_pika.DeliveryMode.PERSISTENT
            )

            # Send the message
            await channel.default_exchange.publish(message, routing_key=PROFILE_REQUEST_QUEUE)
